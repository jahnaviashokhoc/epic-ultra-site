...
.hero h1{font-size:clamp(2.8rem,6vw,5.2rem);font-weight:800;line-height:1.08;letter-spacing:-.04em;margin-bottom:1.5rem}
.hero h1 .accent{color:var(--violet)}
.hero h1 .accent-teal{color:var(--teal)}
.hero-sub{font-size:1.15rem;color:var(--text2);max-width:560px;margin-bottom:.75rem;line-height:1.7}
.hero-support{font-size:.92rem;color:var(--text3);max-width:520px;margin-bottom:2.5rem;font-family:var(--mono)}
...
.section-header h2{font-size:clamp(2rem,4vw,3rem);font-weight:800;letter-spacing:-.04em;line-height:1.1;margin-bottom:1rem}
.section-header p{font-size:1.05rem;color:var(--text2);max-width:560px;line-height:1.7}
...
<section class="hero">
  ...
  <div class="wrap" style="width:100%">
    <div class="hero-content">
      ...
      <h1 class="animate-up-2">Enterprise AI<br>Needs <span class="accent">Architecture</span> —<br>Not Just <span class="accent-teal">Models</span></h1>
...
<section style="padding:5rem 0;background:var(--bg2)">
  <div class="wrap">
    ...
    <div class="arch-stack">
      <div class="arch-layer al-1">
        <span class="hal-dot" style="background:#6c47ff"></span>
        <span class="hal-label">Risk-Grade Data Engineering</span>
        <span class="hal-tag">Reconciled · Audit-ready · Single source of truth</span>
      </div>
      <div class="arch-layer al-2">
        <span class="hal-dot" style="background:#8b6aff"></span>
        <span class="hal-label">Context-Governed Intelligence</span>
        <span class="hal-tag">Structured retrieval · Hallucination reduction</span>
      </div>
      <div class="arch-layer al-3">
        <span class="hal-dot" style="background:#00c4a1"></span>
        <span class="hal-label">Total Decision Traceability</span>
        <span class="hal-tag">Model logging · Policy enforcement logs</span>
      </div>
      <div class="arch-layer al-4">
        <span class="hal-dot" style="background:#00f5c8"></span>
        <span class="hal-label">Mandate-Defined Agent Execution</span>
        <span class="hal-tag">Role-based limits · Financial guardrails</span>
      </div>
      <div class="arch-layer al-5">
        <span class="hal-dot" style="background:#00f5c8;box-shadow:0 0 8px #00f5c8"></span>
        <span class="hal-label">Isolated &amp; Resilient Architecture</span>
        <span class="hal-tag">Versioned containers · Kill-switch controls · Rollback</span>
      </div>
    </div>
  ...
  <div class="wrap">
    ...
    <div class="grid-2">
      <div class="card">
        ...
        <p>Every agent operates within defined mandates. No action executes outside its authorized boundary — ever.</p>
      </div>
      <div class="card">
        <div class="card-icon">🔍</div>
        <h3>Full Explainability</h3>
        <p>Every output includes a complete reasoning chain, data context, and model lineage — audit-ready at any moment.</p>
      </div>
      <div class="card">
        <div class="card-icon">⚡</div>
        <h3>Execution, Not Experimentation</h3>
        <p>EPICUltra is not a pilot. It's production architecture engineered for enterprise accountability from day one.</p>
      </div>
      <div class="card">
        <div class="card-icon">🔒</div>
        <h3>True Isolation</h3>
        <p>Deployment isolation ensures zero cross-contamination between agents, clients, or operational contexts.</p>
      </div>
    </div>
  </div>
</section>
...
<section style="padding:5rem 0">
  <div class="wrap">
    ...
    <div class="grid-3">
      <div class="card">
        <h3 style="color:var(--violet);margin-bottom:1rem">⚡ Energy Trading</h3>
        <p style="margin-bottom:1rem">Real-time exposure management and margin automation for commodity and derivatives trading desks.</p>
        <ul style="list-style:none;display:flex;flex-direction:column;gap:.4rem">
          <li style="font-size:.83rem;color:var(--text2)">→ Intraday exposure monitoring</li>
          <li style="font-size:.83rem;color:var(--text2)">→ Margin automation</li>
          <li style="font-size:.83rem;color:var(--text2)">→ Limit breach detection</li>
          <li style="font-size:.83rem;color:var(--text2)">→ Scenario simulation</li>
        </ul>
      </div>
      <div class="card">
        <h3 style="color:var(--teal);margin-bottom:1rem">🏦 Financial Operations</h3>
        <p style="margin-bottom:1rem">Continuous close, journal automation, and reconciliation intelligence for enterprise finance teams.</p>
        <ul style="list-style:none;display:flex;flex-direction:column;gap:.4rem">
          <li style="font-size:.83rem;color:var(--text2)">→ Journal automation</li>
          <li style="font-size:.83rem;color:var(--text2)">→ Close acceleration</li>
          <li style="font-size:.83rem;color:var(--text2)">→ Reconciliation intelligence</li>
          <li style="font-size:.83rem;color:var(--text2)">→ Variance detection</li>
        </ul>
      </div>
      <div class="card">
        <h3 style="color:var(--violet);margin-bottom:1rem">🛡️ Enterprise Risk</h3>
        <p style="margin-bottom:1rem">Cross-platform exposure aggregation and live risk monitoring for CROs and risk management teams.</p>
        <ul style="list-style:none;display:flex;flex-direction:column;gap:.4rem">
          <li style="font-size:.83rem;color:var(--text2)">→ Anomaly detection</li>
          <li style="font-size:.83rem;color:var(--text2)">→ Counterparty exposure analysis</li>
          <li style="font-size:.83rem;color:var(--text2)">→ Liquidity monitoring</li>
          <li style="font-size:.83rem;color:var(--text2)">→ Policy enforcement</li>
        </ul>
      </div>
    </div>
  </div>
...
# ══════════════════════════════════════════════════════════════════════════════
agents_data = [
    ("📊","Margin Analyst","MONITOR · ANALYZE · ALERT","Real-time margin requirements, collateral levels, and intraday exposure drift","Margin call triggers, collateral substitution recommendations, escalation paths","Hard exposure limits, minimum collateral thresholds, mandate-defined alert levels","CME SPAN · Bloomberg · Internal ERP"),
    ("📡","Exposure Monitor","AGGREGATE · REPORT · ALERT","Cross-platform gross and net exposures, counterparty concentration, sector limits","Concentration breach alerts, hedging recommendations, escalation to risk desks","Counterparty limits, sector exposure caps, regulatory position limits","ICE · Murex · Reuters"),
    ("📝","Journal Entry","EXTRACT · CLASSIFY · POST","Bank statement feeds, transaction queues, and unposted item backlogs","GL account mapping, entry classification, approval routing for exceptions","GL mapping policies, dual-approval thresholds, entity-level posting rules","SAP · Oracle · NetSuite"),
    ("⚖️","Reconciliation","VERIFY · MATCH · RESOLVE","Internal ledger positions vs. custodian statements, bank feeds, clearing house records","Break classification, auto-resolution for known patterns, escalation for unmatched items","Tolerance thresholds, aging limits, mandatory human review for high-value breaks","DTC · SWIFT · Custodian APIs"),
    ("⚡","Close Accelerator","ORCHESTRATE · VALIDATE · CLOSE","Close task completion, open item aging, inter-company eliminations, accrual status","Task sequencing, automated accruals, exception routing, close certification triggers","Close calendar mandates, materiality thresholds, certification authority levels","BlackLine · Workiva · Oracle EPM"),
    ("📋","Regulatory Reporting","COMPILE · VALIDATE · SUBMIT","Reporting deadlines, schema changes, data completeness across submission requirements","Data aggregation logic, schema mapping, validation flags, pre-submission audit review","CFTC/SEC mandates, submission windows, jurisdictional data residency rules","CFTC Portal · SEC EDGAR · FCA"),
    ("💧","Liquidity Watch","FORECAST · MONITOR · ALERT","Intraday cash positions, funding gaps, credit facilities, and settlement obligations","Stress scenario triggers, funding source recommendations, liquidity buffer alerts","LCR/NSFR minimums, internal liquidity buffers, board-approved stress scenarios","Fed Wire · CHIPS · Treasury TMS"),
    ("🧾","Billing Validation","VALIDATE · FLAG · AUDIT","Complex billing calculations, invoice feeds, and billing rule configurations","Validation pass/fail, anomaly flags, escalation to human review","Billing policy rules, approval thresholds, entity-level billing mandates","SAP · Oracle · NetSuite"),
    ("🤖","JAQ – Junior Accountant","DETECT · ANALYZE · EXPLAIN","P&L feeds, variance thresholds, journal entry patterns, and budget vs. actuals","Anomaly alerts, natural-language explanation generation, escalation routing","Materiality thresholds, entity-level reporting rules, review authorization levels","Oracle EPM · Workiva · SAP · Hyperion"),
]

agents_cards = ""
for icon, name, role, monitors, decides, controls, integrations in agents_data:
    role_tags = "".join(f'<span class="role-tag">{r.strip()}</span>' for r in role.split("·"))
    int_tags = "".join(f'<span class="int-tag">{i.strip()}</span>' for i in integrations.split("·"))
    agents_cards += f"""
    <div class="agent-card">
      <div>
        <div style="font-size:1.8rem;margin-bottom:.5rem">{icon}</div>
        <div class="agent-role">{role_tags}</div>
        <h3>{name}</h3>
      </div>
      <div class="agent-detail">
        <div class="agent-detail-row"><strong>Monitors</strong>{monitors}</div>
        <div class="agent-detail-row"><strong>Decides</strong>{decides}</div>
        <div class="agent-detail-row"><strong>Controls</strong>{controls}</div>
      </div>
      <div class="agent-integrations">{int_tags}</div>
    </div>"""
...
<section class="hero" style="min-height:45vh">
  ...
  <div class="wrap" style="width:100%">
    <div class="hero-content">
      ...
      <h1 class="animate-up-2">Nine Agents.<br><span class="accent">Every Detail.</span></h1>
      <p class="hero-sub animate-up-3">Each agent operates within a defined mandate with explicit controls on what it monitors, decides, and executes.</p>
      <div class="hero-stats animate-up-4" style="margin-top:2.5rem">
